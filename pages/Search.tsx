...
import { Search as SearchIcon, Phone, MapPin, MessageCircle, CalendarOff } from 'lucide-react';
...
const Search: React.FC = () => {
  ...
  // --- HELPER: CHECK ELIGIBILITY (Resting Period) ---
  ...
  // --- HELPER: CHECK AGE (Over 18) ---
  const isOver18 = (dobString?: string) => {
    // If no DOB is saved (old users), assume they are over 18 so we don't hide them
  ...
  const filteredDonors = donors.filter(donor => {
    // 1. HIDDEN RULE: If under 18, remove from list completely
          ...
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {filteredDonors.length > 0 ? (
              filteredDonors.map((donor) => {
                ...
                return (
                  <div key={donor.id} className={`bg-white rounded-xl shadow-sm border overflow-hidden hover:shadow-md transition-shadow ${!available ? 'opacity-75 bg-gray-50' : 'border-red-100'}`}>
                    <div className="p-6">
                      <div className="flex justify-between items-start">
                        ...
                        <span className="inline-flex items-center justify-center h-12 w-12 rounded-full bg-red-50 text-red-600 font-bold text-lg border border-red-100">
                          {donor.bloodGroup}
                        </span>
                      </div>
                        ...
                        {available ? (
                          <>
                            <a
                              ...
                              className="flex items-center justify-center px-4 py-2 border border-transparent rounded-lg text-sm font-medium text-white bg-red-600 hover:bg-red-700"
                            >
